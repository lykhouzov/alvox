...
@vertex
fn vs_shadow_main(model: VertexInput, instance: InstanceInput) -> @builtin(position) vec4<f32> {
    ...
    for (var y : i32 = -1 ; y <= 1 ; y = y + 1) {
        for (var x : i32 = -1 ; x <= 1 ; x = x + 1) {
            ...
            shadow = shadow + textureSampleCompare(
                ...
                shadow_pos.z - 0.005  // apply a small bias to avoid acne
            );
        }
    }
    shadow = shadow / 9.0;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // let shadow = fetch_shadow(0u, light.view_proj * in.clip_position);
